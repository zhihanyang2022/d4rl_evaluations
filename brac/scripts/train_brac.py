...
# 500K grad steps; eval_freq is default 5000
flags.DEFINE_integer('total_train_steps', int(5e5), '')  # add/modified by Zhihan: from official repo
flags.DEFINE_integer('bc_train_steps', int(3e5), '')  # added/modified by Zhihan: from official repo
flags.DEFINE_integer('n_eval_episodes', 10, '')
# flags.DEFINE_integer('n_train', int(1e6), '')
flags.DEFINE_integer('value_penalty', 0, '')
flags.DEFINE_integer('pretrain', 0, '')
...
def train_bc():
  ...
  log_dir = os.path.join(
      FLAGS.root_dir,
      mhf.get_agent_dir_for_brac(FLAGS.agent_name, FLAGS.value_penalty),
      FLAGS.env_name,
      str(FLAGS.seed),
      'bc_pretrain',
  ...
  train_eval_offline.train_eval_offline(
      ...
      save_freq=FLAGS.bc_train_steps + 100  # I don't want any models to be saved; added/modified by Zhihan
  )
...
def main(_):
  ...
  log_dir = mhf.get_log_dir(
      base_dir=FLAGS.root_dir,
      algo_dir=mhf.get_agent_dir_for_brac(FLAGS.agent_name, FLAGS.value_penalty),
      env_dir=FLAGS.env_name,
      seed_dir=FLAGS.seed,
  )  # simplified; added/modified by Zhihan

  model_arch = (((300, 300), (200, 200),), 2)
  opt_params = (('adam', 1e-3), ('adam', 3e-4), ('adam', 1e-3))
