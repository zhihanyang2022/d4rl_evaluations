import sys
sys.path.append('bear/')

from gym.envs.mujoco import HalfCheetahEnv, HopperEnv, AntEnv, Walker2dEnv

import rlkit.torch.pytorch_util as ptu
ptu.set_gpu_mode(True)  # optionally set the GPU (default=False); added/modified by Zhihan
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_trains_per_train_loop', default=5000, type=int)  # added/modified by Zhihan; for testing
    args = parser.parse_args()

    print(args.num_trains_per_train_loop)

    # added/modified by Zhihan
    buffer_size = mhf.get_dataset_size(args.env)
    ...
    variant = dict(
        ...
        algorithm_kwargs=dict(
            num_epochs=100,  # num_epochs * num_trains_per_train_loop = 100 * 5K = 500K
            num_eval_steps_per_epoch=10000,  # later on in training, this is about 10 episodes (see max_path_length)
            num_trains_per_train_loop=args.num_trains_per_train_loop,  # do one round of evaluation after every 5K gradient steps
